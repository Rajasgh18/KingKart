...
const Product = ({ details }) => {
    ...
    return (
        <div onClick={handleProduct} className='flex flex-col bottomAppear rounded-lg hover:text-blue-500 gap-2  text-slate-600 justify-start'>
            ...
            <div className='flex flex-col gap-2 bg-white shadow-[0_2px_8px_1px] shadow-slate-300 rounded p-3'>
                ...
                <div className='flex flex-wrap lg:gap-2 md:gap-1 gap-[0.2rem items-end'>
                    <span className='lg:text-lg md:text-sm text-slate-700 font-viga'>Rs {details.offerPrice}</span>
                    <span className='lg:text-base md:text-sm text-slate-500 line-through'>Rs {details.mrp}</span>
                    <span className='lg:text-base md:text-sm text-green-600'>{Math.round(((details.mrp - details.offerPrice) / details.mrp) * 100)}%off</span>
                </div>
            </div>
        </div>
    )
